...
function extractNodesData(jsonData) {
    ...
    // No cluster jewels, no standard nodes
		if(!value.isAscendancyStart && !value.isMultipleChoice && (!value.spc || value.spc.length == 0)) {
			nodeMap[key] = value;
		}
	}
	...
	for( let[key, value] of Object.entries(passiveSkillTreeData.nodes)) {
		...
		if((value.isNotable || value.isKeystone) && (!value.spc || value.spc.length == 0)) {
			...
			if(value.isKeystone) { 
				...
				typeNode = "ascendancy notable";
			} else {
				typeNode = "notable";
			}
			var stats = value.stats;
			if(value.reminderText) stats.concat(value.reminderText);
			nodeMap[value.name] = {reference:value.name, iconUrl: value.icon, data: {sections: {description: stats}, name: value.name, type: typeNode}};
		}
	}
	return nodeMap;
};
