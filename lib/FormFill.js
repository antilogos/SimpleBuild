...
function fillBuildButton(pobData, timestamp, title) {
	...
	let sortedGemsByLink = pobData.gemGroups.flatMap(g => g.groups).sort( (a, b) => a.gems.filter(g => g.enabled).length - b.gems.filter(g => g.enabled).length);
	if(pobData.gemGroups.length > 0 && sortedGemsByLink.length > 0 && sortedGemsByLink.slice(-1)[0].gems.filter(n => !n.skill.startsWith("Support")).length > 0) {
		var gemName;
		gemName = sortedGemsByLink.slice(-1)[0].gems.filter(n => !n.skill.startsWith("Support"))[0].skill;
		var skillGem = allGem[gemName];
		if(skillGem !== undefined && skillGem !== null) {
	...
	historyElement.addEventListener('click', function (event) {
		...
		if(title) window.location.hash = encodeURIComponent(title);
