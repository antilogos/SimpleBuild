...
function pobNoteToHtml(str) {
    ...
    txt.innerHTML = str.trim().replaceAll("\n","<br/>")
		//.replaceAll(/\^x([0-9A-Fa-f]{6})(.*?)\^7/g,"<font color='#$1'>$2</font>") TODO add diferent div for color tag
		.replaceAll(/\^x([0-9A-Fa-f]{6})(.*?)\^7/g,"$2")
...
function addReference(div, references, withLineReturn) {
	if(references.hasOwnProperty(div.getAttribute("id"))) {
		...
		refDiv.textContent = pobNoteToHtml(references[div.getAttribute("id")]);
...
function fillProfile(pobObject) {
	...
	if(pobObject.notes !== null && pobObject.notes.header !== null && pobObject.notes.header.trim().length > 0) { 
		...
		notesDiv.textContent = pobNoteToHtml(pobObject.notes.header);
		notesDiv.classList.add("notesReferences");
