...
function encodeUrl(tree) {
	let nodesBuffer = [];
	let versionBuffer = [4];
	let confBuffer = [];
	confBuffer.push(tree.startClass);
	if(tree.ascendClassId !== undefined) confBuffer.push(tree.ascendClassId); 
	else confBuffer.push(tree.ascendClassId);
...
function buildSvgTree(elementId, treeNodes, classId, ascendClassId) {
	...
	//viewport.setAttribute("viewBox",(minViewX -150) + " " + (minViewY -150) + " " + (maxViewX-minViewX +300) + " " + (maxViewY-minViewY +300));
	while(mainDiv.firstChild) mainDiv.removeChild(mainDiv.firstChild);
	svg.appendChild(viewport);
	mainDiv.appendChild(svg);
	// Bug not rendering the height of the element at start, so we delay the settings of the viewport matrix to center the whole tree
	setTimeout(function () {
		checkLoadingSvg(viewport, [minViewX, minViewY, maxViewX, maxViewX], 100)
	}, 100);
}
...
// Hack to define starting transform matrix using element available width and height (offsetHeight is 0 when element not loaded, and no event can capture the loading)
function checkLoadingSvg(viewport, boundingBox, timer) {
	let mainDiv = viewport.parentElement.parentElement;
	if(mainDiv.offsetHeight != 0) {
		var matrix = new DOMMatrix();
		let spanWidth = Math.abs(boundingBox[0])+Math.abs(boundingBox[2])-450;
		let spanHeight = Math.abs(boundingBox[1])+Math.abs(boundingBox[3])-450;
		matrix.preMultiplySelf(new DOMMatrix()
				.translateSelf(mainDiv.offsetWidth/2, mainDiv.offsetHeight/2)
				.scaleSelf(mainDiv.offsetWidth/spanWidth, mainDiv.offsetHeight/spanHeight)
				.translateSelf(0, 0));
		viewport.style.transform = matrix.toString();	
	} else {
		// use timer parameter to kill recursive loop and delay double if not yet loaded
		if(timer < 6400) {
			setTimeout(function() {
				checkLoadingSvg(viewport, boundingBox, timer*2)
			}, timer*2);
		}
	}	
}
