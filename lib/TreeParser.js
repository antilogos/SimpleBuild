...
function buildPath(nodesObject, elementId, nodeMap, passiveSkillTreeData, customStyle) {
	...
	for( let [key, origin] of Object.entries(filteredNode)) {
		if(origin.x && origin.y) {
			...
			if(nbConnected == 0) {
				console.log(origin, filteredNode);
				const nodePoint = document.createElementNS("http://www.w3.org/2000/svg", "circle");				
				nodePoint.setAttribute("cx", origin.x);
				nodePoint.setAttribute("cy", origin.y);
				if(customStyle) {
	...
	if(popupContainer !== undefined) {
		...
		if(node.stats !== undefined) {
			...
			if(node.reminderText !== undefined) {
				...
				skillMod.textContent = node.reminderText;
				modContainer.appendChild(skillMod);
			}
...
function highlightNode(svg, event) {
	...
	// Find nearest node
	const range = 400;
	let proximity = Object.values(passiveSkillTreeData.nodes).filter( node => node.x !== undefined && node.y !== undefined && node.id !== undefined && !node.isBlighted && !node.isMultipleChoiceOption && node.classStartIndex == null && node.isAscendancyStart == null).filter( node => Math.abs(node.x-x)<range && Math.abs(node.y-y)<range);
	let nearest;
	...
	if(nearest !== undefined) {
		...
		if(event.clientX + popupElement.getBoundingClientRect().width > rect.width) posX = rect.width - popupElement.getBoundingClientRect().width;
...
function scaleZoom(viewport, treeNodes) {
	...
	let displacementLeft = -minViewX;
	let displacementTop = -minViewY;
