...
function buildSvgTree(elementId, treeNodes, classId, ascendClassId) {
	...
	panControls.style.transform = "matrix(0.8, 0, 0, 0.8, "+(viewport.parentElement.getBoundingClientRect().width-50)+", "+(viewport.parentElement.getBoundingClientRect().height/2-50)+")";
	scaleZoom(viewport, treeNodes);
}
...
function buildPath(nodesObject, elementId, nodeMap, passiveSkillTreeData, customStyle) {
	...
	if(panControls.length > 0 && customStyle === undefined) {
		...
		panZoom.style.transform = "matrix(1, 0, 0, 1, 0, 70)";
...
function scaleZoom(viewport, treeNodes) {
	...
	let maxSpace = Math.min(document.body.clientHeight, viewport.parentElement.getBoundingClientRect().width);
	...
	let spanWidth = Math.abs(minViewX)+Math.abs(maxViewX);
	let spanHeight = Math.abs(minViewY)+Math.abs(maxViewY);
	// Then we apply the matrix of the minimum ratio, and translate to the middle (skilltree is centered on 0)
	let ratio = Math.min((maxSpace-50)/spanWidth, (maxSpace-50)/spanHeight);
	let displacementLeft = maxSpace/2 - (minViewX+maxViewX)/2*ratio;
	let displacementTop = maxSpace/2 - (minViewY+maxViewY)/2*ratio;
