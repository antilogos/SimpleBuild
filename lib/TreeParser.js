		...
		if(origin.x && origin.y && origin.out) {
			for( let value of origin.out) {
				if(filteredNode[value]) {
					...
					// Isolated point, probably from unique jewel
					const nodePoint = document.createElementNS("http://www.w3.org/2000/svg", "circle");				
					nodePoint.setAttribute("cx", origin.x);
					nodePoint.setAttribute("cy", origin.y);
					if(customStyle) {
...
function scaleZoom(viewport, treeNodes) {
	...
	let alignmentLeft = margin + ((viewport.parentElement.getBoundingClientRect().width-(spanWidth*ratio))/2);
	let alignmentTop = margin + ((viewport.parentElement.getBoundingClientRect().height-(spanHeight*ratio))/2);
