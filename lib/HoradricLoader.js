			...
			s.gems.forEach( (g, j) => {
				...
				if(skillGem) {
					...
					if(skillGem.base_item !== null) {
						...
						gemSection.properties = [].concat(skillGem.tags.filter(t => GEM_TAG_FILTER["en"].indexOf(t.charAt(0).toUpperCase() + t.slice(1)) >= 0).join(", "));
					} else {
						gemNameId = skillGem.active_skill.display_name;
						gemSection.properties = [].concat(skillGem.active_skill.types.filter(t => GEM_TAG_FILTER["en"].indexOf(t.charAt(0).toUpperCase() + t.slice(1)) >= 0).join(", "));
					}
