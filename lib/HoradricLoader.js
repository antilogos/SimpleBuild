	...
	gemGroups.forEach( (gemGroup, k) => {
		gemGroup.groups.forEach( (s, i) => {
			s.gems.forEach( (g, j) => {
				...
				var skillGem = allGem[g.skill], extraGem;
				...
				if(!skillGem) {
					...
					let extraName; 
					switch (g.skill) {
						case "SupportMomentum": extraName = "Momentum Support"; break;
						case "SupportTrauma": extraName = "Momentum Support"; break;
						case "SupportSpellblade": extraName = "Momentum Support"; break;
						case "SupportReturningProjectiles": extraName = "Momentum Support"; break;
						case "SupportFrigidBond": extraName = "Momentum Support"; break;
						case "SupportSacrifice": extraName = "Sacrifice Support"; break;
						case "SupportGuardiansBlessing": extraName = "Guardian's Blessing Support"; break;
						case "SupportCorruptingCry": extraName = "Corrupting Cry Support"; break;
						case "SupportControlledBlaze": extraName = "Controlled Blaze Support"; break;
						case "SupportVolatility": extraName = "Volatility Support"; break;
						case "SupportSadism": extraName = "Sadism Support"; break;
						case "SupportFreshMeat": extraName = "Fresh Meat Support"; break;
						case "SupportLocusMine": extraName = "Locus Mine Support"; break;
						case "SupportFlamewood": extraName = "Flamewood Support"; break;
						case "SupportDevour": extraName = "Devour Support"; break;
						case "SupportPrismaticBurst": extraName = "Prismatic Burst Support"; break;
						case "SupportManaforgedArrows": extraName = "Manaforged Arrows Support"; break;
						case "Snipe": extraName = "Snipe"; break;
						case "VolcanicFissure": extraName = "Volcanic Fissure"; break;
						case "FrozenLegion": extraName = "Frozen Legion"; break;
						case "SupportCursedGround": extraName = "Cursed Ground Support"; break;
						case "SupportHexBloom": extraName = "Hex Bloom Support"; break;
					}
					if(extraName) extraGem = extraGems.find(g => g.name == extraName);
					if(!extraGem) console.log("No description for skill available:", g.skill);
				}
				...
				var gemNameId;
				if(skillGem && skillGem.base_item !== null) {
					...
					gemNameId = extraGem.name;
					properties.push(extraGem.tags.filter(t => GEM_TAG_FILTER["en"].indexOf(t.charAt(0).toUpperCase() + t.slice(1)) >= 0).join(", "));
				} else {
					// Skill not from gem (unique, enchant, ascendancy)
					gemNameId = g.skill;
				}
				gemSection.properties = properties;
				if(skillGem && skillGem.active_skill !== undefined) {
					// Active skill gem
	...
	treeGroups.forEach( (treeGroup, treeIndex) => {
		...
		if(treeGroup.masteryEffects) {
			...
			Object.entries(masteryGroup).forEach( k => {
				...
				config.reference = "mastery_"+treeIndex+"_"+k[0];
				config.data = nodeData;
				config.iconUrl = "";
				hhData.push(config);
			});
