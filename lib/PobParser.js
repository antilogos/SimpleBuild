...
function pobCodeToDocument(pobCode) {
...
function loadOnceBuildListPob() {
	for (let build of listBuild.filter(b => b.pob)) {
		build.parsed = loadPobData(build.pob);
		if(window.location.hash.length > 0 && window.location.hash.substring(1).toUpperCase() == encodeURIComponent(build.title).toUpperCase()) {
...
function loadPobData(pobObject) {
	var dataPoB = pobCodeToDocument(pobObject);
	// Map of each gems
	var gemGroups = [];
	...
	if(dataPoB.getElementsByTagName("Slot") !== undefined) {
		...
		for (let itemOrSet of itemPart.children) {
			if(itemOrSet.tagName == "Item") {
				...
				item.innerText = itemOrSet.textContent;
	...
	var pobnotes = dataPoB.getElementsByTagName("Notes")[0].textContent;
	var extract = pobnotes.match(/#_([^#]*)#_\n/g);
	if(extract) extract.forEach( match => pobnotes = pobnotes.replaceAll(match, ""));
	var references = {};
	if(extract) extract.map( match => references[match.replaceAll(/#_([\w]*).*\n/g,"$1")] = match.replaceAll(/#_([\w]* )?/g,""));
	var notes = {"header": pobnotes, "refs": references};
	var pobObject = {"gemGroups": gemGroups, "treeGroups": treeGroups, "itemGroups": itemGroups, "notes": notes, "code": pobObject};
	return pobObject;
}
